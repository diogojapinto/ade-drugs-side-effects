...
def FB15kexp(state, channel):
    ...
    print(trainl.shape, trainr.shape, traino.shape)
    timeref = time.time()
    for epoch_count in range(1, state.totepochs + 1):
        ...
        if (epoch_count % state.test_all) == 0:
            ...
            print("\tMEAN RANK >> valid: %s, train: %s" % (
            ...
            print("\t(the evaluation took %s seconds)" % (
