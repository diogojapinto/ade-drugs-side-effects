...
datapath = "D:\\FEUP\\5o_ano\\Investigação\\ade-drugs-side-effects\\drugs-adr-clustering\\data\\"
...
f = open('../data/ADReCS_entity2idx.pkl', 'wb')
g = open('../data/ADReCS_idx2entity.pkl', 'wb')
...
for datatyp in ['train', 'valid', 'test']:
    ...
    inpl = sp.lil_matrix((np.max(list(entity2idx.values())) + 1, len(dat)),
            dtype='float32')
    inpr = sp.lil_matrix((np.max(list(entity2idx.values())) + 1, len(dat)),
            dtype='float32')
    inpo = sp.lil_matrix((np.max(list(entity2idx.values())) + 1, len(dat)),
            dtype='float32')
    ...
    if 'data' not in os.listdir('../'):
        os.mkdir('../data')
    f = open('../data/ADReCS-%s-lhs.pkl' % datatyp, 'wb')
    g = open('../data/ADReCS-%s-rhs.pkl' % datatyp, 'wb')
    h = open('../data/ADReCS-%s-rel.pkl' % datatyp, 'wb')
