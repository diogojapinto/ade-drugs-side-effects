...
def main():
	...
	for drug in validation_set:
		...
		obj = test.random_delete_adrs(obj)
		...
		for e1,e2,label in zip(rbm_pred[0], svd_pred, drug):
			X.append([e1,e2])
			y.append(label)
	...
	for drug in validation_set:
		...
		for e1,e2 in zip(rbm_pred[0],svd_pred):
			prob_prediction.append(regression_threshold.predict([[e1,e2]])[0])

		# Create ROC curve
		fpr, tpr, thresholds = roc_curve(drug, prob_prediction, pos_label = 1)
	...
	for drug in test_set:
		...
		rmse_svd = rmse_svd + rpred.RMSE(svd_pred, drug)
		rmse_rbm = rmse_rbm + rpred.RMSE(rbm_pred[0], drug)
		rmse = rmse + rpred.RMSE(combined_prediction, drug)

		# Create ROC curve
		fpr, tpr, _ = roc_curve(drug, prob_prediction, pos_label = 1)
		roc_auc = auc(fpr,tpr)
		roc_areas.append(roc_auc)

        # Calculate precision recall
		precision, recall, _, _ = precision_recall_fscore_support(drug,combined_prediction, average="macro", pos_label=1)
        ...
        # Calculate precision recall
		precision, recall, _, _ = precision_recall_fscore_support(drug,prob_prediction, average="macro", pos_label=1)
